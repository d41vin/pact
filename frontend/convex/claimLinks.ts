...
// Create claim link
// ⚠️ SECURITY: Private key is stored securely in Convex, only accessible by creator
export const createClaimLink = mutation({
  args: {
    ...
    privateKey: v.optional(v.string()), // Private key (only for "anyone" mode)
  ...
  handler: async (ctx, args) => {
    ...
    const claimLinkId = await ctx.db.insert("claimLinks", {
      ...
      privateKey: args.privateKey, // Store private key for creator to retrieve later
...
export const getClaimLinkByShortId = query({
  ...
  handler: async (ctx, args) => {
    ...
    const isExpired = claimLink.expiresAt
      ? claimLink.expiresAt <= nowInSeconds
      : false;
...
export const recordClaim = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const claimer = await ctx.db
      .query("users")
      .withIndex("by_userAddress", (q: any) =>
        q.eq("userAddress", args.claimerAddress.toLowerCase()),
      )
      .first();
    ...
    await ctx.db.patch(args.claimLinkId, {
      ...
      status:
        isFullyClaimed || isMaxClaimersReached ? "completed" : claimLink.status,
    });

    // Notify creator if they are not the claimer
    console.log(
      `[recordClaim] Checking notification: creator ${claimLink.creatorId} vs claimer ${claimer?._id}`,
    );
...
export const getClaimLinkDetails = query({
  ...
  handler: async (ctx, args) => {
    ...
    const claims = await ctx.db
      .query("claimLinkClaims")
      .withIndex("by_claimLink", (q: any) =>
        q.eq("claimLinkId", args.claimLinkId),
      )
      .collect();
...
export const updateClaimLinkStatus = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    switch (args.action) {
      ...
      case "cancel":
        if (
          claimLink.status === "completed" ||
          claimLink.status === "cancelled"
        ) {
          throw new ConvexError(
            "Cannot cancel completed or already cancelled links",
          );
        }
        newStatus = "cancelled";
        break;
...
export const hasAddressClaimed = query({
  ...
  handler: async (ctx, args) => {
    ...
    const existingClaim = await ctx.db
      .query("claimLinkClaims")
      .withIndex("by_address", (q: any) =>
        q.eq("claimerAddress", args.claimerAddress),
      )
